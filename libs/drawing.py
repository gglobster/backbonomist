...
def BaseDraw(canvas_main, pName, cLen, feat_list) :
    ...
    for feature in feat_list :
        if feature.type == 'CDS' or feature.type == 'cds':
            ORFcnt += 1
            Y_pos = ORFeus(canvas_main, feature, ORFcnt, Y_pos)
...
def ORFeus(canvas_main, feature, count, Y_pos) :
    ...
    canvas_main.setFillColor(color_hex)
...
def ContigDraw(cName, in_file, out_file):
    ...
    annot_feats = [1 for feature in features
                   if feature.qualifiers.get('fct')[0] != 'no match']
    annot_cnt = sum(annot_feats)
